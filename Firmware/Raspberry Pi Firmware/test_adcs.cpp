...
int LASER_ADS1115_HANDLE = getADS1115Handle(0x49);
...
void measure(int loadcellHandle, int laserHandle, AdcBuf &buffer, uint64_t startTime){
	while (!stop_now){
	double loadcellSample = getSample(loadcellHandle);
	double laserSample = getSample(laserSample);
...
int main(){
	...
	setADS1115ContinuousMode(LOADCELL_ADS1115_HANDLE, 0, 0, 7); //loadcell, ch 0, 0 gain (+6.144v), 860 sps
	sleep(0.1);
